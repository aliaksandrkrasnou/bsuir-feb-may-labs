...
function getPolynom() {
    ...
    while (i >= 0) {
        toCount.push({ exp: arguments.length - 1 - i, coef: arguments[i] });
        i--;
    }

    function insidePolynome(x) {
        let result = 0;
        toCount.map(function (val) {
            result += val.coef * Math.pow(x, val.exp);
        });
        return result;
    }
    return insidePolynome;
}
