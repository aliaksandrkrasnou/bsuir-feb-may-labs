...
function getAngleBetweenVectors(x1, y1, x2, y2) {
   let i = Math.acos((x1 * x2 + y1 * y2)/Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2));
   //if (((Math.sign(x1) != Math.sign(x2)) || ()) && ((Math.sign(y1) != Math.sign(y2)) || ())) return Math.PI - i
   return i;
