...
function loadSources(){
    const url = 'https://newsapi.org/v2/sources?apiKey=d51ed79c90564294b6556c7667ce684e';
...
function createNewsItem(token, data){
    token.querySelector('.news__picture').style.backgroundImage = `url(${data.urlToImage ? data.urlToImage : 'Src/image/news.jpg'})`;
...
function loadBy(urlPart){
    hide('#error-block');
    const url = 'https://newsapi.org/v2/' + urlPart + 'apiKey=d51ed79c90564294b6556c7667ce684e';
    const request = new Request(url);
    fetch(request)
        .then(function(response) { return response.json(); })
        .then(function(data) {
